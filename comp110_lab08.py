...
def resize(imageFile):
    ...
    image_window = ImageWin("Original Image", old_image.getWidth(), old_image.getHeight())
    old_image.draw(image_window)

    double_image = double(old_image)
    double_image_window = ImageWin("Resized Image",
                                   double_image.getWidth(),
                                   double_image.getHeight())
...
def halve(oldimage):
    """
    Returns a new image that is a copy of oldImage, but half the size.
    """
...
def display_bird():
    ...
    image_window = ImageWin("Green bird", image.getWidth(), image.getHeight())
...
def make_jailbird(image):
    """
    Returns a copy of image in which regions with green pixels are replaced by
...
def test_jailbird(imageFile):
    ...
    image_window = ImageWin("Original Bird", image.getWidth(),
                            image.getHeight())
    image.draw(image_window)

    new_image = make_jailbird(image)
    new_image_window = ImageWin("Jail Bird", new_image.getWidth(),
                                new_image.getHeight())
